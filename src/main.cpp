...
uint256 hashGenesisBlock("0xba1bb589d892b77c67c3a4d06383c7bd03877172f3d4a9e22fbd12075e1c46c9");
static CBigNum bnProofOfWorkLimit = CBigNum().SetCompact(505876440); // folkArt: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "folkArt Signed Message:\n";
...
{
    // folkArt: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 50); // folkArt: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 180; // folkArt: 3.5 days
static const int64 nTargetSpacing = 90; // folkArt: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // folkArt: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // folkArt: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // folkArt: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4ab51a4dcefc06e4c831bd66d7781837b95800c02b5f82f84add7857c91d4f20");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Cryptsy Recovery FUnd";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 5000000000000;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("2684cdd6a9209c6109d1dc44295e969ebf841536164b8771b3913275b7424d18285b838fc639429411e5801bde4c4e875dcd1952f800ae6ae79ebbffacfe1dfa88") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1454784837;
        block.nBits    = 505876440;
        block.nNonce   = 1956057746;

        if (fTestNet)
        {
            block.nTime    = 1454784837;
            block.nNonce   = 1943869927;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xcb0db5740127ac224459d9075f0eeec220ac1088a9a995ffc45b78cd46503e60"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // folkArt: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("folkArtMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("folkArtMiner : ProcessBlock, block not accepted");
    }
